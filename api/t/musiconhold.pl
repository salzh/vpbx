...
%hash = &send_request(
                        "/pbx/index.pl?action=addmusiconhold",
                        ['recordingfile' => ['/tmp/ivr-accept_reject.wav'],upload_category => 'sale'],                        
                        'uploadfile'
                    );
is($hash{stat}, 'ok', "addrecording: $hash{data}{filename}");


%hash2 = &send_request('/pbx/index.pl?action=getmusiconholdlist');
is($hash2{stat}, 'ok', "getmusiconholdlist ok!");
print Dumper(\%hash2);
