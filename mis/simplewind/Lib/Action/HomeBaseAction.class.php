...
class HomeBaseAction extends AppframeAction {
	...
	function _initialize() {
		parent::_initialize();
		$domains=json_decode(file_get_contents(API_ROOT."index.pl?action=gettenantlist"),true);
		$domains=$domains['data'];
		
		if(empty($_GET['domain_uuid'])){
			
			if(cookie('current_domain')){
				$domain=cookie('current_domain');
			}else{
				$domain=$domains[0]['domain_uuid'];
			}
		}else{
			$domain=$_GET['domain_uuid'];
		}
		
		cookie('current_domain',$domain,864000);
		
		
		$this->assign("controller_url",U(strtolower(GROUP_NAME."/".MODULE_NAME."/index")));
		$this->assign("current_domain",$domain);
